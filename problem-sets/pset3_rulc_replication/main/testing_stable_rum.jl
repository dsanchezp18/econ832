...
tempdir1=@__DIR__
rootdir=tempdir1[1:findfirst("pset3_rulc_replication",tempdir1)[end]]
...
# Doing the charts for the Bootstrap statistics

# Multiply N by the bootstrap statistics

BootN=N.*Boot

# Create a histogram of the bootstrap statistics

histogram(BootN, bins=50, label="Bootstrap statistics", xlabel="Bootstrap statistics", ylabel="Frequency", title="Bootstrap statistics for $(model) model", legend=:topleft)

# Export the histogram to a file

savefig(rootdir*"/main/results/Bootstrap_$(model)_stable.png")