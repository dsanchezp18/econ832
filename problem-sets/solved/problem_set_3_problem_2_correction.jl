...
@everywhere using Distributed, DataFrames, JuMP, Ipopt, Random, Distributions, Statistics
    Random.seed!(78909434) 
    ...
    function bootstrap_sample(df)
        sample = df[rand(1:size(df, 1), size(df, 1)),:]
        return pmap(two_stage_least_squares(sample), [df for i in 1:B])
    end
    B = 10000 # Number of bootstrap samples
    result = @distributed (+) for i in 1:B
        bootstrap_sample(df)
    end
